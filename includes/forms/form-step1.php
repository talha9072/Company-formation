...
<div class="step-form-wrapper">
  <form id="step1form">
    ...
    <!-- HIDDEN COMPANY NAME (AUTO-FILLED AFTER NAME CHECK) -->
...
<script>
/* Only SIC Dropdown UI. Main logic inside form-step1.js */
document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderSicList(category) {
    ...
    Object.entries(codes).forEach(([code, desc]) => {
      ...
      item.innerHTML = `
        <span>${code} - ${desc}</span>
        <button type="button" class="add-sic" data-code="${code}" data-label="${desc}"
    ...
    list.addEventListener("click", function (e) {
      ...
      if (Object.keys(selectedCodes).length >= MAX_SIC_CODES) {
        alert("⚠️ Maximum 4 codes allowed.");
        return;
      }
